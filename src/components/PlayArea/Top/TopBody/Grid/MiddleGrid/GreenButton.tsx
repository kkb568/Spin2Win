...
export default function GreenButton({ chosenNum }: Props) {
    ...
    *  3. correctHover: To show the div with the correctHoverStyle classname.
    ...
    const [buttonState, setButtonState] = useState<buttonStateType>({
        selectedChip: false,
        showTotal: false,
        correctLastBets: getCorrectLastBetsDetails(green, chipsData, lastBetData, previousChosenNums),
        correctHover: false,
    ...
    useEffect(() => {
        if (ifSpinned) {
            updateButtonState("correctLastBets", 
                getCorrectLastBetsDetails(green, chipsData, lastBetData, previousChosenNums)
            )
        }
    }, [ifSpinned])
    ...
    useEffect(() => {
        if (reloadLastBets) {
            ...
            const [ found, lastBetValue ] = checkValueFromLastBet(green, lastBetData);
            if (found) {
